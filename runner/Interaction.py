...
def train_model():
    ...
    user_entity_bias = tf.Variable(tf.random_normal([num_users, 1], mean=0, stddev=0.02), name="user_entity_bias")
    item_entity_bias = tf.Variable(tf.random_normal([num_items, 1], mean=0, stddev=0.02), name="item_entity_bias")
    ...
    user_entity_bs = tf.nn.embedding_lookup(user_entity_bias, users)
    item_entity_bs = tf.nn.embedding_lookup(item_entity_bias, items)
    ...
    J_e_total = (J_e_1 + 0.5 * (J_e_2 - J_e_3)) + user_entity_bs + item_entity_bs

    predict_rating = J_e_total
    loss = tf.reduce_mean(tf.squared_difference(predict_rating, ratings))
    loss += lambda_1 * (tf.nn.l2_loss(user_entity_embedding) + tf.nn.l2_loss(item_entity_embedding) + tf.nn.l2_loss(v_entity) + tf.nn.l2_loss(user_entity_bs) + tf.nn.l2_loss(item_entity_bs))
